...
export const ManagerOverrideModal: React.FC<ManagerOverrideModalProps> = ({
  ...
  const handleAuthorize = async () => {
    if (!managerId) {
      toast.error(t('override.chooseGerant'));
      return;
    }
    if (pin.length < 4) {
      toast.error(t('override.pinRequired'));
      return;
    }
    setIsChecking(true);
    try {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="nova-heading text-base text-foreground">{t('override.title')}</h3>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-muted"
            aria-label={t('override.close')}
          >
        ...
        <div className="mb-4 p-3 rounded-lg bg-muted/40 text-xs space-y-1">
          <p className="text-muted-foreground">
            {t('override.saleFor')} <strong className="text-foreground">{context.productName}</strong> :
          </p>
        ...
        {gerants.length === 0 ? (
          ...
          <>
            {gerants.length > 1 && (
              <div className="mb-3">
                <label className="text-xs text-muted-foreground mb-1 block">{t('override.gerantLabel')}</label>
                <select
                  value={managerId}
                  onChange={e => setManagerId(e.target.value)}
                  className="nova-input w-full py-2"
                >
                  <option value="">{t('override.gerantPlaceholder')}</option>
                  {gerants.map(g => (
        ...
        <div className="flex gap-grid">
          ...
          >
            {isChecking ? t('override.checking') : t('override.authorize')}
          </button>
        </div>
      </div>
    </div>
